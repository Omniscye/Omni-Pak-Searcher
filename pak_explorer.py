...
# Function to perform Mega Scan on XML files with progress bar
def mega_scan(path, search_term, result_text):
    ...
    def do_mega_scan():
        try:
            ...
            result_text.insert(tk.END, f"Mega Scanning {len(pak_files)} .pak files for '{search_term}' in XMLs...\n")
            ...
            mega_scan_results = all_matching_xmls
            if mega_scan_results:
                result_text.insert(tk.END, f"\nMega Scan complete! Found {len(mega_scan_results)} XML files with '{search_term}'.\n")
                ...
                result_text.insert(tk.END, f"No XML files found with '{search_term}'. Total waste of time.\n")
        except Exception as e:
...
def extract_mega_scan_results(result_text):
    ...
    output_dir = filedialog.askdirectory(title="Select Output Directory for Mega Scan Results")
    if not output_dir:
...
class OmniPakSearcherApp(tb.Window):
    ...
    def mega_scan_prompt(self):
        ...
        search_term = simpledialog.askstring("Mega Scan Search Term", "Enter the term to search in XML files:")
        if not search_term:
